...
mod tests {
    ...
    async fn test_quickstart_step7_query_by_type() -> anyhow::Result<()> {
        // Given: Multiple documents of type A exist
        // create_document_auto(&type_a, &user1, None).await?;
        // create_document_auto(&type_a, &user2, None).await?;
        // create_document_auto(&type_b, &user1, None).await?;
    ...
    #[tokio::test]
    #[ignore = "Integration test - will fail until implementation"]
    async fn test_quickstart_step7_query_performance_under_100ms() -> anyhow::Result<()> {
